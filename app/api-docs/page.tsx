...
import { Button, Input, Select, Tooltip, Collapse, Spin } from 'antd';
...
export default function ApiDocsPage() {
  ...
  const queryBuilderUrl = useMemo(() => {
    ...
    if (qbIndicators) params.set('indicators', qbIndicators);
    if (qbEconomies) params.set('economies', qbEconomies);
    if (qbPeriods) params.set('periods', qbPeriods);
    return `${baseUrl}/api/public-explorer/data?${params.toString()}`;
  }, [qbDatasets, qbIndicators, qbEconomies, qbPeriods, baseUrl]);
  ...
  return (
    <div style={{ background: '#f8fafc', minHeight: '100vh', padding: '32px 24px 64px', color: '#1e293b' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        {/* BLUE CALLOUT BANNER (Matching KIDB Style) */}
        <div style={{ background: '#e0f2fe', border: '1px solid #bae6fd', borderRadius: '12px', padding: '20px 24px', marginBottom: '32px', display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
          ...
          <div>
            <h4 style={{ margin: '0 0 4px 0', fontSize: '16px', fontWeight: 700, color: '#0369a1' }}>
              Direct Observation Querying (No Dataflows Required)
            </h4>
            <p style={{ margin: 0, fontSize: '14px', color: '#0c4a6e', lineHeight: 1.5 }}>
              Unlike traditional SDMX APIs that require specifying dataflows (such as <code>EO_NA</code>), the ERDI API allows requesting observation values directly using just <code>indicators</code> and <code>economies</code> codes across all supported datasets.
            </p>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '32px', marginBottom: '40px' }}>
          ...
          <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '28px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
            ...
            <p style={{ color: '#64748b', fontSize: '14px', marginBottom: '20px' }}>
              Query macroeconomic and social observation data values for specified indicators and economies. Returns values alongside units, multipliers, and available periods.
            </p>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '13px' }}>
                ...
                <tbody>
                  <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ padding: '12px 14px', color: '#334155' }}>Primary parameter. Comma-separated indicator code(s) (e.g. <code>POP_TOTL,NGDP_R_CHG</code>).</td>
                  </tr>
                  <tr style={{ borderBottom: '1px solid #f1f5f9' }}>
            ...
            {/* ACCORDION EXAMPLES (Matching KIDB Layout) */}
            <Collapse 
              ...
              items={[
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                      <div>
                        <div style={{ fontSize: '13px', fontWeight: 600, color: '#475569', marginBottom: '6px' }}>
                          1. Query Population series for Philippines (PHI) and Armenia (ARM):
                        </div>
                        <div style={{ background: '#0f172a', padding: '12px 16px', borderRadius: '6px', color: '#38bdf8', fontFamily: 'monospace', fontSize: '13px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <span>GET {baseUrl}/api/public-explorer/data?indicator=POP_TOTL&economy=PHI,ARM</span>
                        </div>
                      </div>

                      <div>
                        <div style={{ fontSize: '13px', fontWeight: 600, color: '#475569', marginBottom: '6px' }}>
                          2. Query multiple indicators across multiple economies for specific years:
                        </div>
                        <div style={{ background: '#0f172a', padding: '12px 16px', borderRadius: '6px', color: '#38bdf8', fontFamily: 'monospace', fontSize: '13px' }}>
                          <span>GET {baseUrl}/api/public-explorer/data?indicators=POP_TOTL,NGDP_R_CHG&economies=ARM,BGD,IND&periods=2020,2021,2022,2023,2024</span>
                        </div>
                      </div>
                    </div>
                  )
                },
                {
                  ...
                  children: (
                    <pre style={{ background: '#0f172a', color: '#f8fafc', padding: '16px', borderRadius: '6px', fontSize: '13px', overflowX: 'auto', margin: 0 }}>
{`{
  "data": [
    {
      "id": "obs-101",
...
params = {
    "indicators": "POP_TOTL,NGDP_R_CHG",
    "economies": "ARM,BGD,IND,PHL",
    "periods": "2020,2021,2022,2023,2024"
}
...
`}
                  ...
                  children: (
                    <div style={{ padding: '8px 0' }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                        ...
                        <div>
                          <label style={{ fontSize: '12px', fontWeight: 600, color: '#475569', display: 'block', marginBottom: '4px' }}>Indicator Codes</label>
                          <Input value={qbIndicators} onChange={e => setQbIndicators(e.target.value)} placeholder="e.g. POP_TOTL" />
                        </div>
                        <div>
                          <label style={{ fontSize: '12px', fontWeight: 600, color: '#475569', display: 'block', marginBottom: '4px' }}>Economy Codes</label>
                      ...
                      <Button 
                        ...
                        onClick={() => {
                          setTestEndpoint(`/api/public-explorer/data?indicators=${qbIndicators}&economies=${qbEconomies}&periods=${qbPeriods}`);
                          handleRunTest();
                        }}
          ...
          {/* STRUCTURAL METADATA ENDPOINTS (Matching KIDB Layout) */}
          <div style={{ background: '#fff', borderRadius: '12px', border: '1px solid #e2e8f0', padding: '28px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
            <h2 style={{ fontSize: '22px', fontWeight: 800, color: '#0f172a', marginBottom: '8px' }}>
              Structural Metadata Endpoints
            </h2>
            <p style={{ color: '#64748b', fontSize: '14px', marginBottom: '24px' }}>
              Retrieve codelists, indicators catalogs, region hierarchies, and definitions.
            </p>
              ...
              <div style={{ border: '1px solid #e2e8f0', borderRadius: '8px', padding: '16px 20px', background: '#fafafa' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <code style={{ fontSize: '14px', fontWeight: 700, color: '#0f172a' }}>/api/public-explorer/indicators</code>
                  </div>
                  <Button size="small" icon={<PlayCircleOutlined />} onClick={() => { setTestEndpoint('/api/public-explorer/indicators'); handleRunTest(); }}>
                    Test Endpoint
                  </Button>
                </div>
                <p style={{ margin: '8px 0 0 0', fontSize: '13px', color: '#64748b' }}>
                  Returns full indicator codelist with dataset codes, definitions, topics, and active economies.
                </p>
              </div>
